...
pub fn ident(parser: &mut parse::Parser, ident: String) -> Result<Expr, r2ks::ParseError> {
    match parser.next() {
        d2k_lexer::R2KSToken::ParenOpen => {
            ...
            loop {
                args.push(match parser.next() {
                    ...
                    _ => return Err(r2ks::Expected::multiple(vec!["Number", "String"]).into()),
                });

                match parser.next() {
                    d2k_lexer::R2KSToken::Comma => (),
                    d2k_lexer::R2KSToken::ParenClose => break,
                    _ => return Err(r2ks::Expected::multiple(vec!["Comma", "ParenClose"]).into()),
                }
            }
        ...
        _ => Err(
            r2ks::Expected::multiple(vec!["If", "Switch", "Variable", "Pointer", "Loop"]).into(),
        ),
    }
}
